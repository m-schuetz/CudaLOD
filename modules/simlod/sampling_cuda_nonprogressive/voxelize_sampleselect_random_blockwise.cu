...
namespace sampleselect_random_blockwise{
	...
	void* sssorted)
{
	...
	// uint64_t* fullVoxelGrids   = nullptr;
	// uint64_t* fullVoxelGrid    = nullptr;

	// create a non-sparse voxel grid to compare with results of colliding-hash-map 
	// if(COMPARE_WITH_NONHEURISTIC){
	// 	fullVoxelGrids = allocator.alloc<uint64_t*>(grid.num_blocks() * numCells * sizeof(uint64_t));
	// 	fullVoxelGrid  = fullVoxelGrids + grid.block_rank() * numCells;
	// }
		...
		while(workIndexCounter < workloadSize){
			...
			for(int childIndex = 0; childIndex < 8; childIndex++){
				...
				for(int it = 0; it < numIterations; it++){
					...
					int ix = fGridSize * (point.x - node->min.x) / boxSize.x;
					int iy = fGridSize * (point.y - node->min.y) / boxSize.y;
					int iz = fGridSize * (point.z - node->min.z) / boxSize.z;
